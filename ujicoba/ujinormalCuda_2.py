...
# VERSI SUPER CEPAT — VRAM < 6 GB, TRAINING < 30 DETIK (100 HARI)
# =============================
...
print("Mixed precision aktif → SUPER CEPAT & HEMAT VRAM!\n")

# ==================================================================
# TOMBOL UTAMA — GANTI DI SINI SAJA! (REKOMENDASI OPTIMAL)
# ==================================================================
USE_REAL_DATA_MODE = False        # False = data identik (bantalan scaler)
N_DUPLICATES       = 100           # boleh 100, 500, bahkan 1000 tetap cepat
WINDOW_DAYS        = 1             # 1 hari = PALING CEPAT & akurat untuk data identik
BATCH_SIZE         = 64            # besar = GPU 100% utilization
MAX_EPOCHS         = 200           # cukup, early stopping akan hentikan lebih cepat
# ==================================================================
...
# =============================
# 7. LABELING SEDERHANA (SEMUA SEHAT KARENA IDENTIK)
# =============================
...
print("Semua hari = SEHAT (data identik) → health_status_per_day.csv disimpan")
...
print(f"Total sample training: {len(X_seq)} (dari {N_DUPLICATES} hari)")
...
if USE_REAL_DATA_MODE:
    ...
    scaler = MinMaxScaler(feature_range=(-0.2, 1.2))   # biar plot 100% sama persis
...
# =============================
# 11. TRAINING SUPER CEPAT
# =============================
dataset = tf.data.Dataset.from_tensor_slices(
    (X_scaled, {'signal': y_signal_scaled, 'status': y_status})
).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)

early_stop = EarlyStopping(monitor='loss', patience=15, restore_best_weights=True, min_delta=1e-6)

print(f"\nMULAI TRAINING ({len(X_scaled)} samples, batch={BATCH_SIZE}) → Tunggu 15-30 detik saja...")
history = model.fit(
    dataset,
    epochs=MAX_EPOCHS,
    verbose=1,
...
mid_points = [(day_boundaries[i] + day_boundaries[i+1]) // 2 for i in range(N_DUPLICATES)]
...
for i, mid in enumerate(mid_points):
    ...
    ax.text(mid, 1.15, "Sehat", ha='center', va='bottom', fontsize=10, fontweight='bold', color='green',
            transform=ax.get_xaxis_transform())

ax.set_xlim(0, len(df_final))
ax.set_title(f"Semua Parameter + Health Status - {N_DUPLICATES} Hari (DATA IDENTIK)", fontsize=14)
ax.set_xlabel("Hari")
...
# Visualisasi 4 hari terakhir (3 gambar)
if len(compressed_dfs) >= 4:
    last_4 = compressed_dfs[-4:]
    df_plot = pd.concat(last_4, ignore_index=True)
    X_full = df_plot[target_columns].values.astype('float32')
    y_true_day4 = compressed_dfs[-1][target_columns].values.astype('float32')

    x_full = np.arange(len(X_full))
    X_norm = scaler.transform(X_full.reshape(-1, n_features)).reshape(X_full.shape)
    y_pred_norm = scaler.transform(pred_signal.reshape(-1, n_features)).reshape(pred_signal.shape)

    day_boundaries = np.arange(0, 5 * POINTS_PER_DAY, POINTS_PER_DAY)
    ...
    def setup(ax, title):
        for pos in day_boundaries[1:]:
            if pos < len(x_full):
                ax.axvline(x=pos, color='red', linestyle='--', linewidth=1.5, alpha=0.9)
        for i, mid in enumerate(mid_points):
            day_num = N_DUPLICATES - 3 + i
            ax.text(mid, 1.05, f'Day {day_num+1}', ha='center', va='bottom', fontsize=12, fontweight='bold', color='red',
                    transform=ax.get_xaxis_transform())
            ax.text(mid, 1.15, "Sehat", ha='center', va='bottom', fontsize=10, fontweight='bold', color='green',
                    transform=ax.get_xaxis_transform())
        ...
        ax.set_xlabel("Titik per hari (1500)")
        ax.set_ylabel("Normalisasi")
        ax.grid(True, alpha=0.3)

    # Gambar 1
    fig, ax = plt.subplots(figsize=(20,8))
    for i, col in enumerate(target_columns):
        ax.plot(x_full, X_norm[:, i], linewidth=0.9, alpha=0.7)
    setup(ax, "GAMBAR 1: 4 Hari Real + Status")
    ...
    handles = []
    for i, col in enumerate(target_columns):
        h = ax.plot(x_full[:WINDOW], X_norm[:WINDOW, i], linewidth=0.9, alpha=0.7, label=col)[0]
        handles.append(h)
    for i, col in enumerate(target_columns):
        ax.plot(x_full[WINDOW:], y_pred_norm[:, i], '--', linewidth=1.8, alpha=0.9)
    setup(ax, "GAMBAR 2: Input + Prediksi 1 Hari")
    ax.legend(handles, target_columns, bbox_to_anchor=(1.02,1), loc='upper left', ncol=2, fontsize='small')
    plt.tight_layout()
    ...
    for i, col in enumerate(target_columns):
        ...
        handles.append(h)
    for i, col in enumerate(target_columns):
        ax.plot(x_full[WINDOW:], X_norm[WINDOW:, i], linewidth=1.2, alpha=0.8)
        ax.plot(x_full[WINDOW:], y_pred_norm[:, i], '--', linewidth=1.8, alpha=0.9)
    setup(ax, "GAMBAR 3: Real vs Prediksi (Day Terakhir)")
    ax.legend(handles, target_columns, bbox_to_anchor=(1.02,1), loc='upper left', ncol=2, fontsize='small')
    plt.tight_layout()
...
print("\nSELESAI 100%! Training < 30 detik, VRAM aman!")
