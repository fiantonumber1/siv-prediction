...
# CEK FILE MODEL
# =========================================================
MODEL_STAGE1  = os.path.join(FORECAST_DIR,   "model_stage1_forecaster.pth")
SCALER_STAGE1 = os.path.join(FORECAST_DIR,   "scaler_stage1.pkl")
MODEL_STAGE2  = os.path.join(CLASSIFIER_DIR, "model_stage2_classifier.pth")
SCALER_STAGE2 = os.path.join(CLASSIFIER_DIR, "scaler_stage2.pkl")
...
# Fallback ke subfolder forecast/ jika tidak ada CSV di root
if len(all_csv_files) < 4:
    all_csv_files = sorted([
        f for f in glob.glob(os.path.join(FORECAST_DIR, "*.csv"))
        if "inference" not in os.path.basename(f).lower()
