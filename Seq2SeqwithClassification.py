...
# =============================
# 13. VISUALISASI: 3 PLOT (SAMA + STATUS)
# =============================
if len(compressed_dfs) < 4:
    ...
    # PERBAIKAN: N+1 boundaries
    day_boundaries = np.arange(0, (n_days_plot + 1) * COMPRESSED_POINTS_PER_DAY, 
                               COMPRESSED_POINTS_PER_DAY)
    
    # PERBAIKAN: mid_points hanya N elemen
    mid_points = [(day_boundaries[i] + day_boundaries[i+1]) // 2 
                  for i in range(n_days_plot)]

    scaler_plot = MinMaxScaler()
    X_flat = X_full.reshape(-1, n_features)
    X_norm = scaler_plot.fit_transform(X_flat).reshape(X_full.shape)
    y_pred_flat = y_pred_day4.reshape(-1, n_features)
    y_pred_norm = scaler_plot.transform(y_pred_flat).reshape(y_pred_day4.shape)
