# =============================
# CLUSTERING HARIAN GLOBAL: 1 HARI = 1 TITIK + CENTROID + LABEL TANGGAL (dd/mm/yyyy)
# Versi FINAL - Lengkap, cantik, informatif + LABEL TANGGAL JELAS
# =============================
...
START_TIME = time(6, 0, 0)      # Jam mulai ambil data
END_TIME   = time(18, 16, 35)   # Jam akhir ambil data
N_DROP_FIRST = 3600             # Skip data awal (misal transient)
...
# ==================================================================
# EKSTRAK TANGGAL DARI NAMA FILE (format: ddmmyyyy.csv)
# ==================================================================
def extract_date(f):
...
def read_and_compress_daily(filepath):
    ...
    date_str = os.path.basename(filepath)[:8]  # ddmmyyyy
...
# ==================================================================
daily_features = []
valid_dates_str = []  # ddmmyyyy
valid_dates_obj = []  # datetime object untuk label cantik
...
for f in csv_files:
    ...
    # Konversi ke datetime untuk label plot
    date_obj = datetime.strptime(date_str, "%d%m%Y")
    valid_dates_obj.append(date_obj)
...
# ==================================================================
# PLOT GLOBAL DENGAN CENTROID + LABEL TANGGAL dd/mm/yyyy
# ==================================================================
...
plt.scatter(centroids_pca[:, 0], centroids_pca[:, 1], 
            c=range(best_k), cmap='tab10', s=1400, marker='*', 
            edgecolors='black', linewidth=2.5, label='Centroid', zorder=10)

# Label centroid
for i, (cx, cy) in enumerate(centroids_pca):
    plt.text(cx, cy, f'  C{i}', fontsize=16, fontweight='bold',
             color='white', ha='left', va='center', zorder=11)

# Label tanggal di tiap titik → FORMAT dd/mm/yyyy (cantik & jelas!)
for i, date_obj in enumerate(valid_dates_obj):
    ...
    plt.text(X_pca[i, 0], X_pca[i, 1], nice_date,
             ...
             rotation=10)  # sedikit miring biar gak tabrakan

# Hitung jumlah hari per cluster
...
plt.xlabel(f"PCA Component 1 ({pca.explained_variance_ratio_[0]:.1%} variance explained)")
plt.ylabel(f"PCA Component 2 ({pca.explained_variance_ratio_[1]:.1%} variance explained)")
...
# ==================================================================
# EXPORT RINGKASAN KE CSV (dengan tanggal dd/mm/yyyy)
# ==================================================================
...
# ==================================================================
# PLOT CONTOH WAVEFORM PER CLUSTER (Output Energy)
# ==================================================================
for cluster_id in range(best_k):
    ...
    for idx in indices:
        ...
        plt.plot(reconstructed[:, 17], alpha=0.6, linewidth=1.2)  # Output Energy = index 17
    
    plt.title(f"Cluster {cluster_id} — {len(indices)} hari — Contoh Waveform Output Energy", fontsize=14)
    plt.xlabel("Waktu dalam hari (6000 titik terkompresi)")
    plt.ylabel("SIV_Output_Energy (rata-rata per chunk)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
print("\n" + "="*90)
print("SELESAI 100%!")
print(f"→ {len(daily_features)} hari berhasil di-cluster menjadi {best_k} pola operasional")
print(f"→ Plot utama (centroid + label tanggal jelas) : {GLOBAL_PLOT}")
print(f"→ Ringkasan cluster per hari (dd/mm/yyyy)     : {SUMMARY_CSV}")
print(f"→ Waveform contoh per cluster                 : folder '{PLOT_DIR}'")
print("="*90)