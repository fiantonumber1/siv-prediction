...
N_DROP_FIRST = 3600
N_TAKE = 150_000
COMPRESSION_FACTOR = 100  # 150k → 1.5k
...
print(f"\nTOTAL DATA SEBELUM KOMPRESI: {len(df_combined):,} baris")
print(f"Hari: {len(all_dfs)} × {POINTS_PER_DAY:,} = {len(df_combined):,}")

# =============================
# 7.5 KOMPRESI 1:100 → 1500 baris/hari
# =============================
print(f"\nMelakukan kompresi 1:100 → dari {POINTS_PER_DAY:,} menjadi {COMPRESSED_POINTS_PER_DAY:,} baris/hari")

# Pastikan 150.000 habis dibagi 100
if POINTS_PER_DAY % COMPRESSION_FACTOR != 0:
    new_points = (POINTS_PER_DAY // COMPRESSION_FACTOR) * COMPRESSION_FACTOR
    print(f"  Peringatan: {POINTS_PER_DAY} tidak habis dibagi {COMPRESSION_FACTOR}.")
    print(f"  Menyesuaikan ke {new_points:,} baris/hari agar kompresi rapi.")
    all_dfs = [df.iloc[:new_points] for df in all_dfs]
    df_combined = pd.concat(all_dfs, ignore_index=True)
    POINTS_PER_DAY = new_points

# Kompresi: rata-rata tiap 100 baris
n_total_rows = len(df_combined)
n_compressed_rows = n_total_rows // COMPRESSION_FACTOR

df_numeric = df_combined[target_columns].astype(float)
df_ts = df_combined['ts_date']
...
for i in range(n_compressed_rows):
    ...
    chunk = df_numeric.iloc[start_idx:end_idx]
    compressed_data.append(chunk.mean())
    
    # Timestamp tengah chunk
    mid_idx = start_idx + COMPRESSION_FACTOR // 2
    compressed_ts.append(df_ts.iloc[mid_idx])

# Buat DataFrame hasil kompresi
df_final = pd.DataFrame(compressed_data, columns=target_columns)
df_final.insert(0, 'ts_date', compressed_ts)
df_final = df_final.reset_index(drop=True)

POINTS_PER_DAY_COMPRESSED = COMPRESSED_POINTS_PER_DAY
n_days = len(all_dfs)

print(f"  SUKSES: {len(df_final):,} baris setelah kompresi")
print(f"  {n_days} hari × {POINTS_PER_DAY_COMPRESSED:,} = {len(df_final):,}")

# =============================
# 8. Simpan CSV (KOMPRESI)
# =============================
...
# =============================
print("\nMembuat plot dengan 1.500 titik per hari...")
...
# Batas hari (setiap 1500 titik)
day_boundaries = np.arange(0, len(df_final), POINTS_PER_DAY_COMPRESSED)
...
# Label Day 1, Day 2, ... di tengah
mid_points = []
for i in range(n_days):
    start = day_boundaries[i]
    end = day_boundaries[i+1] if i < n_days - 1 else len(df_final)
    mid = (start + end) // 2
...
ax.set_title(f"Semua Parameter (Normalisasi) - {n_days} Hari × {POINTS_PER_DAY_COMPRESSED:,} Titik/Hari", fontsize=14)
...
# =============================
print(f"\nSELESAI!")
print(f"SETIAP HARI = {POINTS_PER_DAY_COMPRESSED:,} titik (kompresi 1:100)")
print(f"TOTAL DATA = {len(df_final):,} baris")
print(f"CSV: {output_file}")
print(f"PLOT: plot_all_parameters_1500_per_day.png")
