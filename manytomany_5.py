...
# =============================
# 11. UPSAMPLING (jika perlu, tapi di sini sudah 1500 titik)
# =============================
...
# 13. VISUALISASI: 3 PLOT BESAR ALA "ALL PARAMETERS" (SEMUA 21 KOLOM)
#    - GAMBAR 3: HANYA Real vs Prediksi (TANPA ANOMALI)
# =============================
...
# Fungsi setup plot (sama persis seperti plot_all_parameters)
...
# 3 hari input
for i, col in enumerate(target_columns):
    ax2.plot(x_full[:3*COMPRESSED_POINTS_PER_DAY], X_norm[:3*COMPRESSED_POINTS_PER_DAY, i],
             label=col if i == 0 else "", color=f'C{i}', linewidth=0.9, alpha=0.7)
# 1 hari prediksi
x_day4 = x_full[3*COMPRESSED_POINTS_PER_DAY:]
for i, col in enumerate(target_columns):
    ax2.plot(x_day4, y_pred_norm[:, i], '--', color=f'C{i}', linewidth=1.8, alpha=0.9)
setup_all_params_plot(ax2, f'GAMBAR 2: Semua Parameter – 3 Hari Input + 1 Hari Prediksi')
handles, labels = ax2.get_legend_handles_labels()
ax2.legend(handles[:len(target_columns)], target_columns, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize='small', ncol=2)
plt.tight_layout()
plt.savefig("gambar2_input_plus_prediksi_all_params.png", dpi=300, bbox_inches='tight')
plt.close()

# =============================
# GAMBAR 3: Real vs Prediksi (HANYA Day 4, TANPA ANOMALI)
# =============================
...
for i, col in enumerate(target_columns):
    ax3.plot(x_full[:3*COMPRESSED_POINTS_PER_DAY], X_norm[:3*COMPRESSED_POINTS_PER_DAY, i],
             label=col if i == 0 else "", color=f'C{i}', linewidth=0.9, alpha=0.7)
# Day 4: Real (solid) + Prediksi (dashed)
for i, col in enumerate(target_columns):
    ax3.plot(x_day4, X_norm[3*COMPRESSED_POINTS_PER_DAY:, i], color=f'C{i}', linewidth=1.2, alpha=0.8)
    ax3.plot(x_day4, y_pred_norm[:, i], '--', color=f'C{i}', linewidth=1.8, alpha=0.9)
setup_all_params_plot(ax3, f'GAMBAR 3: Semua Parameter – Real vs Prediksi (Day 4)')
handles, labels = ax3.get_legend_handles_labels()
ax3.legend(handles[:len(target_columns)], target_columns, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize='small', ncol=2)
plt.tight_layout()
...
# PRINT INFO
# =============================
print("\nVISUALISASI #13 SELESAI (ALA ALL PARAMETERS, TANPA ANOMALI DI GAMBAR 3):")
